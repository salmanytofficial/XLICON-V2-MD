...
let handler = async (m, { conn, usedPrefix, command }) => {
  if (!global.lolivid || global.lolivid.length === 0) {
    return conn.sendMessage(m.chat, { text: "No videos found." }, { quoted: m });
  }

  let randomVideoUrl = global.lolivid[Math.floor(Math.random() * global.lolivid.length)];

  await conn.sendMessage(m.chat, { video: { url: randomVideoUrl }, caption: `uWu` }, { quoted: m });
};
